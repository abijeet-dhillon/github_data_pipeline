...
This report summarizes observed performance characteristics, opportunities for optimization, and known issues discovered while developing the COSC 448 GitHub data pipeline and indexing suite.
...
2. **Parallel Repository Processing** – `src.pipeline.runner.process_repo()` runs serially. A worker pool (with safeguards for GitHub rate limits) could shorten end-to-end extraction when targeting many repositories.
...
- **GitHub token exhaustion:** When every Personal Access Token is rate limited, the pipeline sleeps for `RATE_LIMIT_TOKEN_RESET_WAIT_SEC`. Operators should provide multiple fresh tokens to avoid hour-long idle periods.

## Performance Analysis

- **Extraction Throughput:** Using two GitHub tokens and default backoff settings, the pipeline processes a large repository (e.g., `rollup/rollup`) in ~10–12 minutes, driven primarily by commit pagination and blame GraphQL calls. Incremental runs that leverage cached JSON finish significantly faster because only delta issues/commits are requested.
...
Revisit this document as the project evolves—logging new risks and documenting mitigations ensures the pipeline remains reliable for future COSC 448 cohorts and researchers.
