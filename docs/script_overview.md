...
This reference describes the purpose of each script/module, the data it expects, and the service it provides within the GitHub data pipeline and indexing stack.

## Top-Level Entry Points

| Script            | Purpose                                                  | Expected Inputs                                                                   | Service Provided                                                                        |
| ----------------- | -------------------------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| `run_pipeline.py` | Thin CLI wrappers that call `src.pipeline.runner.main()` | Optional CLI repo arguments; GitHub tokens configured in `src/pipeline/config.py` | Launches the data retrieval pipeline without exposing internal modules.                 |
| `run_indexing.py` | Compatibility wrappers for `src.indexing.runner.main()`  | CLI args defined in `src/indexing/config.py`                                      | Bootstraps Elasticsearch indexing to keep backwards compatibility with earlier scripts. |
...
| `runner.py`      | Orchestrates repository processing and file persistence                                     | `process_repo`, `main`                                                                                         | Called by `pipeline.py`/`run_pipeline.py`; ensures every artifact gets written for each repo. |
...
| `schema.py`  | Defines Elasticsearch mappings plus ID helpers and file-to-index routing                    | `MAPPINGS`, `FILE_TO_INDEX`, `stable_hash_id`, `id_*` functions | Keeps index schemas synchronized with the pipeline outputs.                              |
...
| `runner.py`  | Glues configuration, client instantiation, and the scan/index workflow                      | `main`                                                          | Called by `index_elasticsearch.py`/`run_indexing.py`.                                    |
...
| `docs/pipeline_outputs.md`  | Field-by-field explanation of every JSON artifact produced by the pipeline. |
