...
- The script fetches metadata, issues, pull requests, commits, contributors, cross-references, and git blame data, writing JSON artifacts such as `repo_meta.json`, `issues.json`, `pull_requests.json`, `commits.json`, and the new `repo_blame.json`.  
- Commit records include normalized `files_changed`, additions, and deletions so downstream churn analytics have consistent metrics, and every document is stamped with `repo_name` for easier indexing/filtering.  
...
It automatically creates/updates mappings for each document type, ensures `repo_name` is present, and verifies that every artifact generated by `rest_pipeline.py` ends up in its corresponding Elasticsearch index.
